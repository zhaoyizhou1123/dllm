"""
Mask-ProSeCo SFT for LLaDA-8B-Base on rStar-Coder.

Variant of ProSeCo where:
  1. Correction input is model predictions remasked at a random timestep
     (instead of fully revealed predictions).
  2. Both MDM and correction losses are computed on ALL response positions
     (not just masked positions), normalized by L_response.
  3. Lambda weighting: loss = lam * MDM + (1 - lam) * correction,
     where lam linearly decays from 1 to 0 over lambda_warmup_steps.

Two forward passes per training step:
  1. MDM loss: randomly mask response tokens, predict all response positions.
  2. Correction loss: take greedy predictions from pass 1, remask them at a
     fresh random timestep, train model to predict original tokens at all
     response positions.

Local (single GPU):
    accelerate launch --config_file scripts/accelerate_configs/ddp.yaml --num_processes 1 \\
        examples/llada/sft_mask_proseco.py --output_dir .models/mask_proseco

Multi-GPU (FSDP):
    accelerate launch --config_file scripts/accelerate_configs/fsdp2.yaml \\
        examples/llada/sft_mask_proseco.py --output_dir .models/mask_proseco
"""
...
class TrainingArguments:
    output_dir: str = field(default=".models/LLaDA-8B-Base/rstar_coder_mask_proseco")
    ...
    lambda_warmup_steps: int = field(
        default=2000,
        metadata={"help": "Steps over which lambda linearly decays from 1 to 0. "
                  "loss = lam * mdm + (1 - lam) * correction."},
    )
...
# ---------------------------------------------------------------------------
# Validation loss (standard MDM on all response positions)
# ---------------------------------------------------------------------------
...
def compute_val_loss(model, val_loader, mask_id: int, min_t: float, accelerator) -> float:
    ...
    for batch in val_loader:
        ...
        noised, mask_idx, _ = randomly_mask(x0, prompt_mask, mask_id, min_t)
        ...
        L_response = response_mask.float().sum(dim=1, keepdim=True).clamp_min(1)
        loss = (ce * response_mask.float() / L_response).sum() / B
        total_loss += loss * B
        total_count += B
...
def train():
    ...
    # NOTE: do NOT accelerator.prepare(lr_scheduler) — AcceleratedScheduler
    # calls scheduler.step() num_processes times per call (split_batches=False),
    # but total_steps is already computed from the post-prepare (sharded) loader.
    # We step manually inside the training loop when sync_gradients is True.
    ...
    if accelerator.is_main_process and training_args.report_to == "wandb":
        accelerator.init_trackers(
            ...
            config={
                ...
                "lambda_warmup_steps": training_args.lambda_warmup_steps,
                "total_steps": total_steps,
            },
            init_kwargs={"wandb": {"name": "dllm-llada-mask-proseco"}},
        )
    ...
    # ---- Training loop -------------------------------------------------------
    logger.info(
        f"Starting Mask-ProSeCo training: total_steps={total_steps}, "
        ...
        f"lambda_warmup_steps={training_args.lambda_warmup_steps}"
    )
    ...
    while global_step < total_steps:
        ...
        lam = max(0.0, 1.0 - global_step / training_args.lambda_warmup_steps)

        # Precompute response mask and normalization (shared by both losses)
        response_mask = ~prompt_mask  # [B, L]
        L_response = response_mask.float().sum(dim=1, keepdim=True).clamp_min(1)  # [B, 1]

        with accelerator.accumulate(model):
            # ---- Pass 1: MDM loss ----
            noised, mask_idx, _ = randomly_mask(
            ...
            # Loss on ALL response positions, normalized by L_response
            mdm_loss = (ce_mdm * response_mask.float() / L_response).sum() / B

            if lam > 0:
                accelerator.backward(lam * mdm_loss)

            # ---- Pass 2: Mask-ProSeCo correction ----
            with torch.no_grad():
                pred = logits.detach().argmax(dim=-1)   # [B, L]
            # Construct revealed sequence: prompt from x0, response from pred
            revealed = torch.where(prompt_mask, x0, pred)
            # Remask the revealed sequence at a fresh random timestep
            corr_noised, _, _ = randomly_mask(
                revealed, prompt_mask, MASK_ID, training_args.min_t
            )

            corr_logits = model(input_ids=corr_noised).logits  # [B, L, V]
            ce_corr = F.cross_entropy(
                corr_logits.transpose(1, 2), x0, reduction="none"
            )  # [B, L]
            # Loss on ALL response positions, normalized by L_response
            correction_loss = (ce_corr * response_mask.float() / L_response).sum() / B

            if (1.0 - lam) > 0:
                accelerator.backward((1.0 - lam) * correction_loss)
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % training_args.logging_steps == 0:
                    total_loss = lam * mdm_loss.item() + (1.0 - lam) * correction_loss.item()
                    logs = {
                    ...
                    if accelerator.is_main_process:
                        logger.info(
                            ...
                            f"lam={lam:.3f}  lr={lr_scheduler.get_last_lr()[0]:.2e}"
                        )
