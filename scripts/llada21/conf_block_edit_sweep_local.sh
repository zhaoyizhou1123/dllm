#!/usr/bin/env bash
#SBATCH --job-name=eval_llada21_conf_block_edit_sweep
...
# edit_step scaling sweep for the LLaDA2.1-mini HumanEval accuracy-vs-NFE curve.
# 2-D SLURM array: 7 edit_step values x 8 chunks = 56 tasks (--array=0-55).
#   task_id / 8 -> index into EDIT_STEPS ; task_id % 8 -> chunk id.
# All other hyperparameters are held at the reproduced headline config
# (threshold=0.9, early_exit_number=2, ...). Derived from conf_block_edit_full_local.sh.
