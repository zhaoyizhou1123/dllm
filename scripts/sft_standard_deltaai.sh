...
#SBATCH --mem=200G
#SBATCH --time=2:00:00
#SBATCH --output=slurm/standard_sft/job_%A.out
...
export WANDB_PROJECT=llada-sft

# ---- Paths ---------------------------------------------------------------
DATA_DIR=/projects/bgqz/zzhou24/data/rstar_coder
OUTPUT_DIR=/work/nvme/bgqz/zzhou24/checkpoints/standard_rstar

# ---- Cluster config ------------------------------------------------------
# GH200 nodes: up to 4 GPUs per node (ghx4).
# 4 GPUs: global batch = 4 × 4 × 8 = 128
NGPUS=4
GRAD_ACCUM=8
...
srun --nodes=1 --ntasks=1 \
  accelerate launch \
    ...
    examples/llada/sft.py \
      ...
      --save_steps 500 \
      --save_total_limit 1 \
      --run_name dllm-llada-standard \
      --report_to wandb
      # --resume_from_checkpoint "$OUTPUT_DIR/checkpoint-XXXX"
