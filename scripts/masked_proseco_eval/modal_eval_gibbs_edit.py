"""
Evaluate a masked-proseco LLaDA-8B checkpoint on HumanEval using Modal (single H100).

Uses Gibbs-edit bidirectional block diffusion (GibbsSampler with edit_strategy=gibbs_edit).
...
Run:
    modal run scripts/masked_proseco_eval/modal_eval_gibbs_edit.py

Custom checkpoint / edit_step:
    modal run scripts/masked_proseco_eval/modal_eval_gibbs_edit.py \
        --checkpoint-path /checkpoints/mask_proseco_rstar/step_600 \
        --edit-step 8
"""
...
app = modal.App("dllm-llada-eval-gibbs-edit")
...
def evaluate(
    ...
    edit_step: int = 8,
    edit_start: int = 32,
    edit_strategy: str = "gibbs_edit",
    remasking_strategy: str = "random",
    early_exit_number: int = 1,
    num_workers: int = 4,
    output_dir: str = "",
):
    import os
    ...
    if not output_dir:
        output_dir = (
            f"/eval_results/humaneval_maxlen{max_new_tokens}_block{block_size}/"
            f"gibbs_edit_bidir_block{block_size}_steps{steps}"
            f"_editstep{edit_step}_editstart{edit_start}_global-earlyexit"
        )
    ...
    model_args = (
        ...
        f"edit_freq={edit_freq},"
        f"edit_step={edit_step},"
        f"edit_start={edit_start},"
        f"edit_strategy={edit_strategy},"
        f"remasking_strategy={remasking_strategy},"
        f"early_exit_number={early_exit_number},"
